...
class BaseFigure(object):
    ...
    def __init__(
        ...
        self._data = [orjson.loads(orjson.dumps(trace._props)) for trace in data]
        ...
        self._layout = orjson.loads(orjson.dumps(self._layout_obj._props))
    ...
    def data(self, new_data):
        ...
        for i, trace in enumerate(self.data):
            if id(trace) in remove_uids:
                ...
                old_trace._orphan_props.update(orjson.loads(orjson.dumps(old_trace._props)))
...
""".format(
    ...
    def add_traces(
        ...
        new_traces_data = [orjson.loads(orjson.dumps(trace._props)) for trace in data]
...
""",
    ...
    def layout(self, new_layout):
        ...
        new_layout_data = orjson.loads(orjson.dumps(new_layout._props))

        # Unparent current layout
        # -----------------------
        if self._layout_obj:
            old_layout_data = orjson.loads(orjson.dumps(self._layout_obj._props))
...
""".format(
    ...
    def to_dict(self):
        ...
        # Handle layout
        # -------------
        layout = orjson.loads(orjson.dumps(self._layout))

        # Handle frames
        # -------------
        # Frame key is only added if there are any frames
        res = {"data": data, "layout": layout}
        frames = [orjson.loads(orjson.dumps(frame._props)) for frame in self._frame_objs]
...
class BasePlotlyType(object):
    ...
    def _set_compound_prop(self, prop, val):
        ...
        if curr_val is not None:
            curr_dict_val = orjson.loads(orjson.dumps(curr_val._props))
        else:
            curr_dict_val = None

        if val is not None:
            new_dict_val = orjson.loads(orjson.dumps(val._props))
        else:
            new_dict_val = None
    ...
    def _set_array_prop(self, prop, val):
        ...
        if curr_val is not None:
            curr_dict_vals = [orjson.loads(orjson.dumps(cv._props)) for cv in curr_val]
        else:
            curr_dict_vals = None

        if val is not None:
            new_dict_vals = [orjson.loads(orjson.dumps(nv._props)) for nv in val]
        else:
            new_dict_vals = None
...
""".format(
    ...
    def to_plotly_json(self):
        ...
        dict
        """
        return orjson.loads(orjson.dumps(self._props if self._props is not None else {}))
